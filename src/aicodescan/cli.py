...
app = typer.Typer(help="Context-aware AI code security scanner.", no_args_is_help=True)
...
@app.callback()
def main() -> None:
    """Context-aware AI code security scanner."""
...
def _provider_from_config(config: AppConfig) -> Provider | None:
    ...
    try:
        ...
        raise typer.BadParameter(f"unsupported provider in .aicodescan.yaml: {config.provider}") from exc
...
def _exit_with_summary(summary: ProjectScanSummary, output: str) -> None:
    ...
    if summary.files_scanned == 0:
        stdout_console.print(Panel("No Python files were scanned.", title="aicodescan"))
        raise typer.Exit(code=0)
